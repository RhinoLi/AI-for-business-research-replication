...
if not c.env.computer == 'YOUR_WORKSTATION_NAME':
    if c.experiment_type == 'combined_model_train':
        ...
        c.env.CUDA_VISIBLE_DEVICES = '0,1,2,3' # Number of GPUs to use. CUDA device identifiers.
    else:
        c.env.CUDA_VISIBLE_DEVICES = '1' # Number of GPUs to use. CUDA device identifiers.
else:
    c.env.CUDA_VISIBLE_DEVICES = os.environ['CUDA_VISIBLE_DEVICES']

# The c.noise setting determines the latent space distribution, and is newly added.
c.noise = 'sphere'  # Standard setting for this paper's Progressive GAN

# The c.empty_cache setting can help with memory fragmentation on some GPUs.
# Set to True if you encounter out-of-memory errors during training.
c.empty_cache = True
...
c.use_cometML = False # CometML is a SaaS provider of ML Training and model tracking.
c.comet_api_key = ""
...
# The above learning rate is too high when training 64 by 64 images,
# Adjustment is needed.
c.e_lr = 0.0005 # Encoder Learning Rate
c.g_lr = 0.0005
...
c.use_ram_for_image_load = False # Load images directly into RAM?  Usually faster than SSD memmap.
...
# When training 64 by 64, c.match_x_scaleneeds to be reduced to 2.0
...
try:
    ...
    c.gpu_count = 4  # Set manually multi-gpu
...
# These commented codes are original versions.
# elif c.dataset == 'chairs':
...
#     c.dining_room_chair_labeled_inds_dir = "/path/to/your/dining_room_chair_labeled_inds.csv"
    ...
    # Define the root data directory for easy management
    base_data_path = "/mnt/disk5/home/leranli/project/ms21_product_aesthetic_design_replication_files/data"
    
    # Results will be saved in a 'results' folder within your project directory
    c.result_dir = os.path.join(os.getcwd(), "results")
    
    # Dataset Paths - updated with the absolute paths to your files
    c.images_dir = os.path.join(base_data_path, "chair_data_grayscale.h5")
    c.masks_dir = os.path.join(base_data_path, "chair_data_masks.h5")
    
    # Index and Rating Files
    c.ratings_dataset_path = os.path.join(base_data_path, "chair_ratings_traditional_modern.csv") 
    c.dining_room_chair_full_inds_dir = os.path.join(base_data_path, "dining_room_chair_full_inds.csv")
    c.dining_room_chair_labeled_inds_dir = os.path.join(base_data_path, "dining_room_chair_labeled_inds.csv")
...
if c.load_checkpoint:
    checkpoint_base = "/mnt/disk5/home/leranli/project/ms21_product_aesthetic_design_replication_files/results/002-combined_model_train_modern_NewRun_iter2000000_seed0_1.0fracdata/checkpoint/"
    # Update the specific filename string below when you actually have a saved state
    c.load_checkpoint_dir = os.path.join(checkpoint_base, "3524480_state")
...
# Automatically point to a 'checkpoints' folder in your project
c.load_checkpoint = True  # Set to True only when resuming a crashed job
if c.load_checkpoint:
    checkpoint_base = "/mnt/disk5/home/leranli/project/ms21_product_aesthetic_design_replication_files/results/002-combined_model_train_modern_NewRun_iter2000000_seed0_1.0fracdata/checkpoint/"
    # Update the specific filename string below when you actually have a saved state
    c.load_checkpoint_dir = os.path.join(checkpoint_base, "3524480_state")
...
def batch_size_dict(reso):
    """
    Returns batch size based on resolution. 
    Adjusted for GTX 4090 (24GB VRAM per card).
    """
    if c.gpu_count == 1:
        # Standard table for 24GB VRAM
        batch_table = {4: 128, 8: 128, 16: 128, 32: 64, 64: 32, 128: 16, 256: 8, 512: 4, 1024: 1}
    
    elif c.gpu_count == 2:
        # Optimized for your SLURM request (2x 4090s)
        # We increase these slightly compared to 1 GPU to utilize both cards
        batch_table = {4: 256, 8: 256, 16: 256, 32: 128, 64: 64, 128: 32, 256: 16, 512: 8, 1024: 2}
        
    elif c.gpu_count >= 4:
        # Scale-up table for when you move to 8 GPU "Big Money" mode
        batch_table = {4: 512, 8: 512, 16: 512, 32: 256, 64: 128, 128: 64, 256: 32, 512: 16, 1024: 4}
    
    else:
        # Fallback for 3 or unsupported GPU counts
        batch_table = {4: 128, 8: 128, 16: 64, 32: 32, 64: 16, 128: 8, 256: 4, 512: 2, 1024: 1}

    return batch_table.get(reso, 8) # Default to 8 if resolution not found
