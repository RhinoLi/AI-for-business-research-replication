    ...
    with session.comet_experiment.train() if session.c.use_cometML else ExitStack():
        while session.sample_i < total_steps:
            ...
            if session.c.use_masks:
                ...
                # To this:
                # This "squashes" the Tanh output (-1, 1) into a Sigmoid-like range (0, 1) 
                # so the loss function doesn't crash.
                mask_loss = mask_loss_criterion(torch.sigmoid(recon_mask.squeeze(1)), batch_masks)
            ...
            if labeled_data:
                ...
                p_loss.backward()
                # --- ADD CLIPPING HERE ---
                # Clip gradients for the Encoder and Predictive model
                torch.nn.utils.clip_grad_norm_(session.generator.encoder.parameters(), max_norm=1.0)
                torch.nn.utils.clip_grad_norm_(session.generator.predictive_model.parameters(), max_norm=1.0)
